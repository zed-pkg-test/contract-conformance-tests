...
Related PRs: zed-pkg/zed-interfaces#90 and zed-pkg/zed-lib-core#41.
...
integer spelling cases, with Clippy and the original root Cargo lock. Those Rust
cases are production-owned, while the 111 extra TypeScript probes are independently
owned here; neither is represented as universal equivalence evidence.
...
and unchanged. The lib-core pin initially names the immutable implementation PR
head; replace it with the tested merge commit before final certification.
